...
import { ModalCrearBanco } from "../components/banco/ModalCrearBanco";

export const PageBanco = () => {
  const { getBancos, bancos } = useBanco();
      ...
      <div className="flex">
        <div className="flex flex-col gap-1 bg-white py-6 px-8 max-full">
          <p className="text-gray-700 text-sm font-bold">
            Total de la caja/banco
          </p>
          <p className="font-bold text-2xl text-blue-500">
            {formatearDinero(bancos[0]?.saldo)}
          </p>
        </div>
      </div>
      ...
      >
        <div>
          <Link className="flex gap-3 text-sm font-bold bg-blue-500 text-white rounded-full py-2 px-5 hover:bg-orange-500 transition-all items-center">
            Ver movimientos del banco del mes
            <FaArrowRight />
          </Link>
        </div>
        ...
        <div>
          <Link className="flex gap-3 text-sm font-bold bg-blue-500 text-white rounded-full py-2 px-5 hover:bg-orange-500 transition-all items-center">
            Ver ingresos o gastos del banco
            <FaArrowRight />
          </Link>
        </div>
        ...
        <div>
          <Link className="flex gap-3 text-sm font-bold bg-blue-500 text-white rounded-full py-2 px-5 hover:bg-orange-500 transition-all items-center">
            Filtrar ingresos o gastos del banco.
            <FaArrowRight />
          </Link>
        </div>
        ...
        {bancos.length === 0 ? (
          <div>
            <button
              ...
              onClick={() => {
                // Aquí puedes manejar la lógica para mostrar tu modal
                document.getElementById("my_modal_crear_banco").showModal();
              }}
            >
              Crear nueva banco
            </button>
