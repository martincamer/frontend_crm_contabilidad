...
export const ModalEditarEmpleadoAguinaldo = ({ idObtenida }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      document
        .getElementById("my_modal_editar_empleado_mobile_aguinaldo")
        .close();
    } catch (error) {
  ...
  return (
    <dialog id="my_modal_editar_empleado_mobile_aguinaldo" className="modal">
      <div className="modal-box rounded-none w-full h-full max-h-full">
        ...
        <div className="px-0 py-5 w-full min-h-full bg-white flex flex-col gap-5">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {termino_pago === "quincenal" ? (
              <>
                <div className="font-bold text-sm mb-3 mt-5">
                  <p>Datos del aguinaldo</p>
                </div>

                <div className="w-1/3" onClick={handleInputClick}>
                  {isEditable ? (
                    <FormInput
                      labelText={"Banco"}
                    ...
                    <div className="flex flex-col gap-1 w-full">
                      ...
                      <p className="border capitalize border-[#E2E8F0] bg-[#F7FAFC] py-[0.90rem] px-[0.75rem] focus:border-blue-500 rounded-none outline-none outline-[1px] text-xs font-semibold">
                        {formatearDinero(Number(aguinaldo_proporcional) || 0)}
                      </p>
                    </div>
                  )}
                </div>

                <div className="font-semibold bg-blue-100/50 text-blue-500 py-5 px-5 flex flex-col gap-2 mt-3">
                  <p className="flex gap-2 items-center">
                    Aguinaldo total
                    <span className="font-extrabold bg-white py-1 px-2">
                      {" "}
                      {formatearDinero(
                        Number(totalSueldo / 2) - Number(aguinaldo_proporcional)
                      )}
                    </span>
                  </p>
                </div>
                ...
                <div className="font-semibold bg-blue-100/50 text-blue-500 py-5 px-5 flex flex-col gap-2">
                  <p className="flex gap-2 items-center">
                    ...
                    <span className="font-extrabold bg-white py-1 px-2">
                      {" "}
                      {formatearDinero(
                        Number(totalSueldoNeto / 2) -
                          Number(aguinaldo_proporcional)
                      )}
                    </span>
                  </p>
                </div>
            ...
            <div className="mb-5">
              <Submit>Actualizar aguinaldo</Submit>
            </div>
          </form>
        </div>
      </div>
