...
export const CrearNuevosSectores = () => {
  const { createSectores, sectores, getSectores } = useEmpleado();
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      await createSectores(datos);

      document.getElementById("my_modal_crear_sectores").close();
  ...
  return (
    <dialog id="my_modal_crear_sectores" className="modal">
      <div className="modal-box rounded-none max-w-2xl">
        ...
        >
          <label className="flex flex-col gap-2">
            <span className="font-bold">Nombre del sector</span>
            <input
